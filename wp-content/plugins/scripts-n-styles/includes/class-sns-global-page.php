...
class SnS_Global_Page
{
	...
	static function enqueue_scripts( $value, $page, $action, $new, $old ) {
		...
		if ( ! empty( $value['enqueue_scripts'] ) && ! array_intersect( $value['enqueue_scripts'], Scripts_n_Styles::get_wp_registered() ) == $value['enqueue_scripts'] ) {
			add_settings_error( $page, 'settings_updated', __( 'Invalid value for Enqueue Scripts.' ), 'error' );
			return $old;
		}
		return $value;
	}
