...
class WPEditorLog {
  ...
  public static function log( $data ) {
    if ( defined( 'WPEDITOR_DEBUG' ) && WPEDITOR_DEBUG ) {
      ...
      $filename = WPEDITOR_PATH . '/'.$log_file_name.'-log.txt'; 
  ...
  public static function get_log_file_path() {
    $log_file_name = self::get_log_file_name();
    $log_file_path = WPEDITOR_PATH . '/'.$log_file_name.'-log.txt';
    return $log_file_path;
  }
  ...
  public static function get_log_file_name() {
    $log_file_option = 'wpe_log_file_name';
    $log_file_name = get_option($log_file_option);
    if ( false === $log_file_name ) {
      $log_file_name = bin2hex(random_bytes(8));
      add_option($log_file_option, $log_file_name);
    }
    return $log_file_name;
  }
  
  public static function get_log_file_url() {
    return WPEDITOR_URL . self::get_log_file_name() . '-log.txt';
  }
